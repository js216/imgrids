...
target/sdl/.built: $(RS)
	...
	CARGO_TARGET_DIR=target/sdl cargo build --release --features sdl,bpp16 $(EX_FLAGS)
...
target/fb32/.built: $(RS)
	CARGO_TARGET_DIR=target/fb32 cargo build --release --features fb0,bpp32 $(EX_FLAGS)
	mkdir -p target/release/examples
...
target/armv7/.built: $(RS)
	...
	cargo build --release --features fb0,bpp16 $(EX_FLAGS) --target armv7-unknown-linux-gnueabihf
...
target/wasm/.built: src/sim.html $(RS)
	...
	cargo build --release --features web,bpp32rgba $(EX_FLAGS) --target wasm32-unknown-emscripten
